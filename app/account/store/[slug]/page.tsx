...
import { StoreProductCard } from "@/app/store/[slug]/store-product-card";

type Search = { q?: string; category?: string };

export default async function AccountStorePage({
  params,
  searchParams,
}: {
    ...
    <div>
        ...
        action={
          <Button variant="outline" asChild>
            <Link href="/account/sellers">
              <ArrowLeft className="h-4 w-4" /> Sellers
            </Link>
          </Button>
        }
      />
      ...
      <Card className="mb-6">
        <CardContent className="p-4">
          <form className="flex flex-col gap-3 sm:flex-row" action={`/account/store/${slug}`}>
